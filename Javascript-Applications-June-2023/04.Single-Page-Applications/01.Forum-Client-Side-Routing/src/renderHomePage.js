...
const homeTemplate = (ctx, onFormSubmit, onFormClear, topicsArr) => ctx.html`
...
<div class="container">
    <main>
        ...
        <div class="topic-title">
            <!-- topic component  -->
            ${topicsArr.length ? topicsArr.map(topic => createTopicTemplate(ctx.html, topic)) : null}
        </div>
...
function renderHomePage(ctx) {
    getTopicsData()
        .then(topicsData => {
            ...
            ctx.render(homeTemplate(ctx, onFormSubmit, onFormClear, topicsArr), ctx.root);
        })
        .catch(err => {
